    ...
    <div class="pwa-form page-enter">
      ...
      <form id="residuForm">
        ...
        <div class="form-group" id="locationGroup" style="display:${(userDesa || isOperatorTPS || institusiHasFacility) ? 'block' : 'none'}">
          <label class="form-label">${(isOperatorTPS || institusiHasFacility) ? 'Fasilitas Terdaftar' : 'Lokasi TPS3R / Bank Sampah (Opsional)'}</label>
          <select id="locationSelect" class="form-select" ${(isOperatorTPS || institusiHasFacility) ? 'disabled' : ''}>
            ...
            ` : `
              ...
              ${userDesa ? locations.filter(l => (l.desa_id === userDesa.id || (Array.isArray(l.served_desa_ids) && l.served_desa_ids.includes(userDesa.id))) && [...FACILITY_TYPES, ...INSTITUTION_TYPES].includes(l.type)).map(l => `<option value="${l.id}" data-lat="${l.lat}" data-lng="${l.lng}" data-name="${l.name}">${l.name}</option>`).join('') : ''}
            `}
          </select>
        </div>
        ...
        <div class="form-group">
          ...
          <select id="destinationSelect" class="form-select">
            <option value="tpa" selected>TPA - Controlled Landfill</option>
